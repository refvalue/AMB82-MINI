...
namespace BinaryUtil {
    constexpr uint16_t readU16Be(std::span<const uint8_t, 2> buffer) noexcept {
        return (static_cast<uint16_t>(buffer[0]) << 8) | static_cast<uint16_t>(buffer[1]);
    }

    constexpr uint32_t readU32Be(std::span<const uint8_t, 4> buffer) noexcept {
    ...
    constexpr uint64_t readU64Be(std::span<const uint8_t, 8> buffer) noexcept {
    ...
    constexpr void writeU16Be(std::span<uint8_t, 2> buffer, uint16_t value) noexcept {
        buffer[0] = static_cast<uint8_t>(value >> 8);
        buffer[1] = static_cast<uint8_t>(value & 0xFF);
    }

    constexpr void writeU32Be(std::span<uint8_t, 4> buffer, uint32_t value) noexcept {
    ...
    constexpr void writeU64Be(std::span<uint8_t, 8> buffer, uint64_t value) noexcept {
