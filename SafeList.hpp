 ...
 */
template <typename T>
class SafeList : public AbstractList<T> {
    /*!
...
public:
     ...
     */
    SafeList() : AbstractList<T>{false} {}

    SafeList(const SafeList&) = delete;

    SafeList(SafeList&& other) noexcept : AbstractList<T>{this->isMutable()}, head{other.head}, tail{other.tail} {
        other.head = nullptr;
        other.tail = nullptr;

        for (size_t i = 0; i < static_cast<size_t>(other.getSize()); i++) {
            this->increaseSize();
        }
    ...
    SafeList& operator=(SafeList&& right) noexcept {
        auto temp = right.head;
        ...
        const auto size1 = static_cast<size_t>(this->getSize());
        const auto size2 = static_cast<size_t>(right.getSize());
